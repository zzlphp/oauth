...
$html_detail = htm_fields ($htmsd,'iphone</td>','iphone</td>','&nbsp;&nbsp;&nbsp;&nbsp;');
...
for($j=1;$j<=$cxnums;$j++){
	...
	$sql = "SELECT COUNT(*) FROM `phone_log` WHERE title='".$tmp['title']."' and time='".date('Y-m-d')."'";
	...
	if($res[0]==0){
		$sql = "INSERT `phone_log`(`title`,`price`,`time`) VALUES('".$tmp['title']."',".$tmp['price'].",'".date('Y-m-d')."')";
